...
class ApplyFormal extends Model
{
    protected $table = 'apply_formals';
    ...
    public function storeData($inputs, $setting_info){
    	//申请表
    	$formal = new ApplyFormal;
        $formal->user_id = $inputs['user_id'];
    	$formal->dept_id = $inputs['dept_id'];
    	$formal->work_content = $inputs['work_content'];
    	$formal->work_ok = $inputs['work_ok'];
    	$formal->work_learn = $inputs['work_learn'];
        $formal->work_plan = $inputs['work_plan'];
        $formal->formal_date = $inputs['formal_date'];
        $formal->status = 0;
        ...
        if(!empty($step1_setting)){
            //申请无限制  走正常流程
            $formal->status_txt = $step1_setting->name;//步骤说明
            $next_user_ids = array();
            ...
            $formal->current_verify_user_id = implode(',', $next_user_ids);
            $formal->step = $step1_setting->step;
        }

        
		DB::transaction(function () use($formal,$setting_info,$user_data){
		    $formal->save();
    		//流程表
	    	$audit_process = new \App\Models\AuditProces;
	    	$audit_process->storeData($setting_info, $formal, 6, 'ApplyFormal');
	    	//往通用表里面加一条数据
	    	$apply_main = new \App\Models\ApplyMain;
	    	$apply_main->storeData($formal, 6, 'ApplyFormal', '转正申请');
		}, 5);
    	$re = true;
    	return $re;
    }
